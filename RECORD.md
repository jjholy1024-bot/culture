...
### 12. 호주 팝업창 지도 이탈 방지 위치 고정 (6차 세션)
- **개선**: 호주(AUS)의 영토가 매우 넓어 지도 경계 근처를 클릭할 경우 팝업창이 화면/지도를 벗어나는 문제를 해결했습니다.
- **수정**: [main.js](file:///D:/public_data/culture/main.js) 내 `EDGE_SAFE_ISO`에 `AUS`를 추가하고, `FIXED_POPUP_ANCHOR`에 호주 대륙 서부 부근 좌표(`[-25.0, 125.0]` - 기존 133.0에서 피드백을 반영해 왼쪽으로 추가 조정)를 설정하여 날짜변경선 인근이거나 줌 레벨이 2~3 이하일 때 고정 좌표로 팝업이 안정적으로 띄워지도록 수정했습니다.
- **캐시**: `index.html` 내 `main.js` 및 `style.css` 호출부의 캐시버스터를 `?v=20260705k`로 업데이트하였습니다.

### 13. 파푸아뉴기니, 일본 슬라이드 효과 적용 (7차 세션)
- **개선**: 파푸아뉴기니(PNG) 및 일본(JPN)의 지도 팝업이 열릴 때 또는 일부지역 경보 마커 클릭 시, 뉴질랜드(NZL)와 마찬가지로 지도가 해당 국가 위치로 부드럽게 이동하는 슬라이드(panTo) 효과를 적용하였습니다.
- **수정**: [main.js](file:///D:/public_data/culture/main.js) 내 팝업 오픈 및 마커 클릭 리스너의 조건문에 `PNG`와 `JPN`을 추가했습니다.
- **캐시**: `index.html` 내 `main.js` 및 `style.css` 호출부의 캐시버스터를 `?v=20260706a`로 업데이트하였습니다.

### 14. 전체 국가 슬라이드 효과 확대 적용 (8차 세션)
- **개선**: 슬라이드 효과를 특정 국가군에만 한정하지 않고, 유효한 위경도 좌표(`c.lat && c.lng`)가 있는 모든 국가에 대해 지도 팝업 오픈 및 마커 클릭 시 지도가 해당 국가로 부드럽게 이동(panTo)하도록 확대 적용하였습니다.
- **수정**: [main.js](file:///D:/public_data/culture/main.js) 내 팝업 오픈 및 마커 클릭 리스너의 조건문을 특정 국가 체크에서 위경도 유효성 체크로 변경했습니다.
- **캐시**: `index.html` 내 `main.js` 및 `style.css` 호출부의 캐시버스터를 `?v=20260706b`로 업데이트하였습니다.
